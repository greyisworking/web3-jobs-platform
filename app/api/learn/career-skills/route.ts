...
export async function GET(request: NextRequest) {
  ...
  const sortedSkills = Object.entries(skillCounts)
    ...
    .map(([skill, count]) => ({
      skill,
      count,
      percentage: Math.round((count / matchingJobs.length) * 100),
    }))
  ...
  return NextResponse.json({
    ...
    skills: {
      core: coreSkills,      // 30%+ of jobs mention
      common: commonSkills,  // 15-30% mention
      niceToHave: niceToHave, // 5-15% mention
      all: sortedSkills,
    },
