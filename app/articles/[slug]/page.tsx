...
export default function ArticleDetailPage() {
  ...
  const handleCollect = async () => {
    ...
    setCollecting(true)
    try {
      // In real implementation, this would trigger an on-chain mint or record collection
      ...
      setCollected(true)
      setArticle(prev => prev ? { ...prev, collect_count: prev.collect_count + 1 } : null)
    } catch (err) {
  ...
  const handleTip = async (amount: number) => {
    ...
    // Update tip amount in UI
    setArticle(prev => prev ? {
      ...prev,
      tip_amount: prev.tip_amount + amount
    } : null)

    // In real implementation, this would record the tip after blockchain tx
    try {
